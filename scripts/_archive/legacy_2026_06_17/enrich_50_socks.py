...
# Load top 50 and top 15 (already enriched)
with open('/mnt/ssd/1688-only/data/socks_top50.json') as f:
    top50 = json.load(f)['top50']
with open('/mnt/ssd/1688-only/data/socks_top15_enriched.json') as f:
    t15 = json.load(f)
t15_ids = set(p['offer_id'] for p in t15['products'])
print(f'SOCKS top 50: {len(top50)}, already enriched: {len(t15_ids)}', flush=True)

# Get the new products only
...
# Merge: top 15 enriched + new enriched
all_products = t15['products'] + new
with open('/mnt/ssd/1688-only/data/socks_top50_enriched.json', 'w', encoding='utf-8') as f:
